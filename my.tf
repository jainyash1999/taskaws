...
resource "aws_instance" "web1" {
 ...
 connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = file("/root/Desktop/key/keyfolder/my11.pem")
...
resource "null_resource" "image" {
  ...
  provisioner "local-exec" {
     ...
     command = " git clone https://github.com/jainyash1999/taskaws.git images"
...
resource "aws_s3_bucket_object" "object" {
  bucket = aws_s3_bucket.yash11.bucket
  acl = "public-read"
  key    = "terraform.png"
  source = "images/terraform.png"
depends_on=[aws_s3_bucket.yash11,null_resource.image]
...
resource "aws_cloudfront_distribution" "my_distribution" {
    origin {
         domain_name = "${aws_s3_bucket.yash11.bucket_regional_domain_name}"
         origin_id   = "${aws_s3_bucket.yash11.id}"
...
default_cache_behavior {
    ...
    target_origin_id = "${aws_s3_bucket.yash11.id}"
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/jainyash1999/taskaws.git /var/www/html/",
      "sudo su << EOF",
